...
export class BaPictureUploader {
  ...
  public companyLogoData:Object;
  public logoUploaded:boolean = false;

  @ViewChild('fileUpload') protected _fileUpload:ElementRef;

  public uploadInProgress:boolean = false;
  public logoUpload:boolean = false;
  ...
  protected _onUploadCompleted(data):void {
    ...
    this.companyLogoData = JSON.parse(data.response);
    if(this.companyLogoData.status < 0){
      toastr.error(this.companyLogoData.data.message);
      this.logoUpload = false;
    }else{
      toastr.success(this.companyLogoData.data.message);
      localStorage.setItem('company_logo',this.companyLogoData.data.name);
    }
